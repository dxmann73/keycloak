...
- Configure Firefox as follows by writing `about:config` in the address bar
  - network.auth.use-sspi: false
    - this will disable SSPI for Firefox. SSPI is a Windows-specific proprietary variant of the IETF standard GSSAPI, which is used by the MIT Kerberos client 
  ...
  - Note: Be aware to turn back these settings if you want Firefox to use proper Windows Active Directory-based SSO again
